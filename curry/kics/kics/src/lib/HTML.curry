...
--- <a href="http://www.informatik.uni-kiel.de/~mh/publications/papers/PADL01.html">
...
form title hexps = HtmlForm title [BodyAttr ("bgcolor","#ffffff")] hexps
...
textfield (CgiRef ref) contents = -- ref argument will be instantiated
    HtmlCRef
...
password (CgiRef ref) = -- ref argument will be instantiated
    HtmlCRef
...
textarea (CgiRef ref) (height,width) contents =
    HtmlCRef
...
checkbox (CgiRef ref) value = -- ref argument will be instantiated
    HtmlCRef
...
checkedbox (CgiRef ref) value = -- ref argument will be instantiated
    HtmlCRef
       (HtmlStruct "input" [("type","checkbox"),("name",ref),
...
radio_main (CgiRef ref) value = -- ref argument will be instantiated
    HtmlCRef
...
radio_main_off (CgiRef ref) value = -- ref argument will be instantiated
    HtmlCRef
       (HtmlStruct "input" [("type","radio"),("name",ref),
                            ("value",htmlQuote value)] [])
       (CgiRef ref)
...
radio_other (CgiRef ref) value = -- ref argument will be instantiated
    HtmlStruct "input"
...
selection (CgiRef ref) menue = -- ref argument will be instantiated
    HtmlCRef
...
selectionInitial (CgiRef ref) sellist sel =
...
multipleSelection (CgiRef ref) sellist =
...
showHtmlPage (HtmlPage title params html) =
  ...
  showHtmlExp (HtmlStruct "html" htmlTagAttrs
                  [HtmlStruct "head" []
                   ...
                   HtmlStruct "body" [("bgcolor","#ffffff")] html])
 where
...
runFormServerWithKeyAndFormParams url cgikey formparams hformact = do
 ...
 where
  ...
  startCgiServer timeout port scriptkey = do
    time <- getClockTime
    let initstate = (0, nextCleanup time, [])
    (state,htmlstring) <- computeFormInStateAndEnv True url cgikey formparams
                                               initstate scriptkey hformact []
    putStr htmlstring
    hClose stdout
    let portname = port++scriptkey
    ...
    serveCgiMessagesForForm timeout url cgikey formparams hformact socket state

-- The default timeout period for the cgi server in milliseconds:
defaultCgiServerTimeout = 7200000 -- two hours

-- The main server loop:
serveCgiMessagesForForm :: Int -> String -> String -> [FormParam]
                        -> IO HtmlForm -> Socket -> ServerState -> IO ()
serveCgiMessagesForForm servertimeout url cgikey fparams initform socket =
  serveCgiMessages
 where
  serveCgiMessages state =
    waitForSocketAccept socket servertimeout >>=
    maybe (do -- terminate server due to inactivity
              ltime <- getLocalTime
              putErrLn $ calendarTimeToString ltime ++
                         ": terminated due to timeout")
          (\ (rhost,hdl) ->
             if take 8 rhost /= "127.0.0." && rhost /= "localhost"
             then hClose hdl >> serveCgiMessages state
             else readCgiServerMsg hdl >>= 
  ...
  serveCgiMessage state hdl ShowStatus =
   ...
   where
    showEventHandler (key,time,_,(_,handler)) = do
      ltime <- toCalendarTime time
      return $ "No. " ++ show key ++ ", expires at " ++
               calendarTimeToString ltime ++ ": " ++
               showAnyQExpression handler ++ "\n"

  serveCgiMessage state hdl SketchStatus =
    reportStatus state hdl sketchEventHandler
   where
    sketchEventHandler (key,time,_,_) = do
      ltime <- toCalendarTime time
      return $ "No. " ++ show key ++ ", expires at: " ++
               calendarTimeToString ltime ++ "\n"
  ...
  reportStatus state@(maxkey,ctime,ehs) hdl eh2string = do
    lctime <- toCalendarTime ctime
    ehsstrings <- mapIO eh2string ehs
    hPutStrLn hdl $ "Next cleanup: " ++ calendarTimeToString lctime ++
                    " (maxkey: " ++ show maxkey ++")\n"++
...
computeFormInStateAndEnv isinitform url cgikey fparams state scriptkey
    ...
    nstate <- storeEnvHandlers state (encodeKey cgikey)
...
noHandlerPage cgiurl urlparam =
  "Content-Type: text/html\n\n" ++ showForm [] "" (noHandlerDoc urlparam)
 where
  noHandlerDoc uparam = expires 0 $ standardForm "Error: no submission handler"
   [par [htxt "Your request cannot be processed due to one of the following reasons:"],
    ulist
     [[htxt "You have not submitted the web form for a long period (timeout)."],
      [htxt "You have used the 'back' button of your browser and submitted the ",
       htxt "web form again (which should not be done in order to avoid the ",
       htxt "double submission of data)."],
      [htxt "The web server has been rebooted."]],
    par [htxt "In any case, ",
         href (cgiurl ++ if null uparam then "" else '?':uparam)
              [htxt "please click here to restart."]]]
...
extractCookies (HtmlForm title params hexp) =
 ...
 where
   ...
   splitFormParams [] = ([],[])
   splitFormParams (FormCookie n v ps : fps) =
     let (cs,ops) = splitFormParams fps in ((n,v,ps):cs,ops)
   splitFormParams (FormCSS s : fps) =
     let (cs,ops) = splitFormParams fps in (cs,FormCSS s : ops)
   splitFormParams (FormJScript s : fps) =
     let (cs,ops) = splitFormParams fps in (cs,FormJScript s : ops)
   splitFormParams (FormOnSubmit s : fps) =
     let (cs,ops) = splitFormParams fps in (cs,FormOnSubmit s : ops)
   splitFormParams (FormTarget s : fps) =
     let (cs,ops) = splitFormParams fps in (cs,FormTarget s : ops)
   splitFormParams (HeadInclude h : fps) =
     let (cs,ops) = splitFormParams fps in (cs,HeadInclude h : ops)
   splitFormParams (BodyAttr s : fps) =
     let (cs,ops) = splitFormParams fps in (cs,BodyAttr s : ops)
...
intFormMain baseurl basecgi reldir cginame forever urlparam hformact = do
  ...
  time <- getClockTime
  let initstate = (0, nextCleanup time, [])
  intFormInEnv url cgikey hformact hformact [] initstate forever socket
...
--             (index,expiration date,cgikey,env,handler)
type ServerState =
  (Int, ClockTime, [(Int,ClockTime,String,([(String,String)],HtmlHandler))])

--- Stores a list of new event handlers for a given cgi program and
--- the corresponding arguments with a new key.
storeEnvHandlers :: ServerState -> String -> [(String,String)]
                 -> [(HtmlHandler,String)] -> IO ServerState
storeEnvHandlers ostate cgikey env handlerkeys = do
  time <- getClockTime
  cstate <- cleanOldEventHandlers ostate
  let nstate = generateEventServerMessages cstate (eventHandlerExpiration time)
                 handlerkeys
  seq nstate done -- to ensure that handler keys are instantiated
  return nstate 
 where
   generateEventServerMessages state _ [] = state
   generateEventServerMessages state expiredate ((handler,hkey):evhs)
     | show (keyOfState state) ++ ' ':showQTerm (toUTCTime expiredate) =:= hkey
     = generateEventServerMessages
             (storeNewEnvEventWithCgiKey state expiredate env handler)
             expiredate evhs

   keyOfState (key,_,_) = key

   storeNewEnvEventWithCgiKey (maxkey,cleandate,ehs) date cenv info
    = (if maxkey>30000 then 0 else maxkey+1, -- to avoid integer overflows
       cleandate,
       (maxkey,date,cgikey,(cenv,info)):ehs)

-- clean event handlers that are too old:
cleanOldEventHandlers :: ServerState -> IO ServerState
cleanOldEventHandlers state@(_,_,[]) = return state
cleanOldEventHandlers state@(maxkey,cleandate,ehs@(_:_)) = do
  ctime <- getClockTime
  if compareClockTime ctime cleandate == LT
   then return state
     ...
     return (maxkey, nextCleanup ctime, currentehs)
 where
  isNotExpired time (_,etime,_,_) = compareClockTime time etime == LT
  ...
  let (numstring,datestring) = break (==' ') skey
   ...
   in if null datestring || null dateps || num < 0
      then return (state,Nothing)
            ...
            in return (newstate, info)
 where
  getEnvEventWithCgiKey (maxkey,cleandate,ehs) key date =
    if singleUseEventHandlers
    then let (dehs,r) = deleteEv ehs in ((maxkey, cleandate, dehs), r)
    else ((maxkey,cleandate,ehs), searchEv ehs)
   where
     deleteEv [] = ([],Nothing)
     deleteEv (ev@(n,t,c,i):es) =
       if key==n && date == toUTCTime t
       then if c==cgikey then (es, Just i)
                         else (ev:es, Nothing)
       else let (des,r) = deleteEv es in (ev:des, r)
  
     searchEv [] = Nothing
     searchEv ((n,t,c,i):es) = if key==n && date == toUTCTime t
...
registerCgiServer url pname = exclusiveIO (cgiServerRegistry++".lock") $ do
