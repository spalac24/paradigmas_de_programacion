module ExternalFunctionsUnsafe where

import Curry
import CurryPrelude
import ExternalDataMeta
...
{-
spawnConstraint seems impossible without very internal support.

spawnConstraint C_Success x = x
spawnConstraint (C_SuccessOr ref xs) x = C_SuccessOr ref (map (\ (a,b) -> (a,spawnConstraint b x)) xs)
spawnConstraint s@(C_SuccessSusp r cont) x = hnf x
  case consKind x of
    Failed    -> failed (C_ErrorCall (toCurry "spawnConstraint") :< exceptions x)
    Branching -> 
    Val -> let sRef = nextRef () 
            in suspend sRef 
                 (ctcTreatSusp sRef 
                    (\ store s' -> spawnConstraint s' x) s)
    
spawnConstraint (C_SuccessFail es) _ = failed (C_ErrorCall (toCurry "spawnConstraint") :< es)
spawnConstraint v@(C_SuccessFreeVar ref) x = C_SuccessOr (nextRef ()) [(modStore v C_Success,x)]

susp :: Curry a => SuspRef -> C_Success -> a -> ConsKind -> a
susp sRef s@(C_SuccessSusp ref cont) x Val = 
  suspend sRef (\store -> case cont Store 

s'@(C_SuccessSusp ref' cont') =

susp C_Success x = x
susp (C_SuccessOr ref xs) x = C_SuccessOr ref (map (\ (a,b) -> (a,susp b x)) xs)
susp 
   C_SuccessSusp ref (\ store -> cont' store `op_38` cont store)
susp (C_SuccessFail es) _ = C_SuccessFail (C_ErrorCall (C_Char '&' :< List) :< es)
susp v@(C_SuccessFreeVar ref) x = C_SuccessOr ref [(modStore v C_Success,x)]
