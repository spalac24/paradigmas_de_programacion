...
module CPNS(registerPort,getPortInfo,unregisterPort,cpnsAlive) where
...
tryRegisterPortName regs name pid sn pn = do
  ...
  return (ack, (name,pid,sn,pn) : filter (\ (n,_,_,_)->name/=n) regs)

-- Delete all registrations for a given port name:
unregisterPortName regs name = do
  ctime <- getLocalTime
  putStrLn $ "Port \""++name++"\" unregistered at "++calendarTimeToString ctime
  return (filter (\ (n,_,_,_)->name/=n) regs)
...
cpnsTryGetAnswer host msg = catchFail tryGetAnswer connectError
 where
  tryGetAnswer = do
  ...
  connectError = error $ "Curry port name server at host \""++host++
                         "\" is not reachable!"
...
cpnsAlive timeout host = catchFail tryPingCPNS (return False)
 where
  tryPingCPNS = do
    h <- connectToSocket host cpnsSocket
