...
module MetaProgramming.FlatCurry 
                 (Prog(..), QName, Visibility(..),
...
readFlatCurry fn = do
   let filename = if hasExtension fn then fn else replaceExtension fn ".fcy"
   readModule filename >>= return . read . skipComment
  where
